"""
Load and Test ViT-B/16 Transfer Model on CIFAR-10
...
MODEL_PATH = MODEL_DIR / "ViT_B16_Transfer_best.pth"
...
def make_model(num_classes=10):
    """
    학습 코드와 동일한 torchvision ViT-B/16 구조 생성.

    테스트에서는 weights=None 사용:
    - 이미 ViT_B16_Transfer_best.pth 안에 학습된 weight가 저장되어 있음
    - 다시 ImageNet pretrained weight를 다운로드할 필요 없음
    """
    model = vit_b_16(weights=None)

    # ImageNet 1000 class head → CIFAR-10 10 class head
...
def load_model_weights(model, model_path, device):
    if not model_path.exists():
        raise FileNotFoundError(
            ...
            f"예상 위치:\n"
            f"CIFAR10_project/saved_models/ViT_B16_Transfer_best.pth"
        )
